...
After each merge, the orchestrator tries to delete the remote branch (`git push origin --delete todo/X`), but GitHub's "auto-delete head branches" setting already removed them. This produces a noisy warning on every item:

```
Warning: Failed to delete remote branch todo/M-OBS-1: git push failed (exit 1): error: unable to delete 'todo/M-OBS-1': remote ref does not exist
```
...
2. **Auto-detect:** Before deleting, check if the remote branch exists (`git ls-remote --heads origin todo/X`). Skip if already gone.
