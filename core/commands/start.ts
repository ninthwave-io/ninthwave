...
/** Supervisor agent files to seed (source in agents/, targets in project). */
const SUPERVISOR_AGENT_TARGETS = [
  { dir: ".claude/agents", filename: "supervisor.md" },
  { dir: ".opencode/agents", filename: "supervisor.md" },
  { dir: ".github/agents", filename: "supervisor.agent.md" },
];

/**
 * Seed supervisor agent file into the project root if not already present.
 * Copies from the hub repo's agents/supervisor.md.
 */
export function seedSupervisorAgent(projectRoot: string, hubRoot: string): void {
  const sourcePath = join(hubRoot, "agents", "supervisor.md");
  if (!existsSync(sourcePath)) return;
  const sourceContent = readFileSync(sourcePath, "utf-8");

  for (const target of SUPERVISOR_AGENT_TARGETS) {
    const destPath = join(projectRoot, target.dir, target.filename);
    if (existsSync(destPath)) continue;
    mkdirSync(dirname(destPath), { recursive: true });
    writeFileSync(destPath, sourceContent);
  }
...
/** Context passed to the supervisor as its initial message. */
export interface SupervisorContext {
  items: { id: string; state: string; workspaceRef?: string; prNumber?: number; title: string }[];
...
/**
 * Build the initial message sent to the supervisor session.
 * Contains the full pipeline state so the supervisor has context from the start.
 */
export function buildSupervisorInitialMessage(ctx: SupervisorContext): string {
  const itemLines = ctx.items.map((item) => {
    let line = `- ${item.id}: state=${item.state}, title="${item.title}"`;
    if (item.workspaceRef) line += `, workspace=${item.workspaceRef}`;
    if (item.prNumber) line += `, PR=#${item.prNumber}`;
    return line;
  }).join("\n");

  const workspaceLines = ctx.items
    .filter((i) => i.workspaceRef)
    .map((i) => `- ${i.id}: ${i.workspaceRef}`)
    .join("\n");

  return `[ORCHESTRATOR] Supervisor session started.
...
## Configuration
- Merge strategy: ${ctx.mergeStrategy}
- WIP limit: ${ctx.wipLimit}
${ctx.frictionDir ? `- Friction directory: ${ctx.frictionDir}` : ""}

## Workspace References
${workspaceLines || "(no active workspaces)"}

Monitor these workers and report anomalies. You will receive event messages as items transition states and a periodic heartbeat with full state.`;
}

/**
 * Launch a supervisor Claude Code session.
 *
 * Key differences from launchAiSession():
 ...
 */
export function launchSupervisorSession(
  projectRoot: string,
  mux: Multiplexer,
  aiTool: string,
  context: SupervisorContext,
  hubRoot: string = projectRoot,
): string | null {
  // Seed supervisor agent file if not already present
  seedSupervisorAgent(projectRoot, hubRoot);

  let cmd = "";
  let initialMessage = buildSupervisorInitialMessage(context);

  switch (aiTool) {
    case "claude":
      cmd = `claude --name 'Supervisor' --permission-mode bypassPermissions --agent supervisor`;
      break;
    case "opencode":
      cmd = `opencode --agent supervisor --title 'Supervisor'`;
      break;
    case "copilot": {
      const launcherScript = `/tmp/nw-supervisor-launch-${Date.now()}.sh`;
      const promptDataFile = `/tmp/nw-supervisor-prompt-${Date.now()}`;
      writeFileSync(promptDataFile, initialMessage);
      writeFileSync(
        launcherScript,
        `#!/bin/bash\nPROMPT=$(cat '${promptDataFile}')\nrm -f '${promptDataFile}' '${launcherScript}'\nexec copilot --agent=supervisor --allow-all -i "$PROMPT"\n`,
      );
      run("chmod", ["+x", launcherScript]);
      cmd = launcherScript;
      initialMessage = ""; // embedded in cmd via -i
      break;
    }
    default:
      warn(`Cannot launch supervisor: unknown AI tool "${aiTool}"`);
      return null;
  }

  const wsRef = mux.launchWorkspace(projectRoot, cmd, "supervisor");
  if (!wsRef) {
    warn(`Supervisor launch failed — is ${mux.type} running?`);
    return null;
  }

  // Skip send when the prompt was embedded in the launch command (copilot)
  if (!initialMessage) return wsRef;

  const sleep: (ms: number) => void =
    process.env.NODE_ENV === "test" ? () => {} : (ms) => Bun.sleepSync(ms);
  const delivered = sendWithReadyWait(mux, wsRef, initialMessage + "\n", sleep);
  if (!delivered) {
    warn("Supervisor prompt delivery failed — falling back to legacy send");
    if (!waitForReady(mux, wsRef)) {
      warn("Supervisor workspace did not become ready — sending prompt anyway");
    }
    if (!mux.sendMessage(wsRef, initialMessage + "\n")) {
      warn("Failed to send initial prompt to supervisor");
    }
  }
